...
// ProgramCodeLogin implements service.IAuth.
func (s *sAuth) ProgramCodeLogin(ctx context.Context, req *dto_auth.ProgramCodeLogin) (res *dao_auth.LoginRes, err error) {

	if req.Provider == consts.WEIXIN {
		obj, err := s.weiXinCode(ctx, req)
		if err != nil {
			return nil, err
		}
...
func (s *sAuth) weiXinCode(ctx context.Context, req *dto_auth.ProgramCodeLogin) (res *dao_auth.LoginRes, err error) {
	...
	if userDetail.IsEmpty() {
		...
		_, err = dao.SysUser.Ctx(ctx).
			Where(dao.SysUser.Columns().Id, userDetail.GMap().Get(dao.SysUser.Columns().Id)).
			Data(g.Map{
