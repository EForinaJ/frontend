...
const VITE_UPLOAD_BASEURL = `${getEnvBaseUrl()}/upload/file`
...
export default function useUpload<T extends TfileType>(options: TOptions<T> = {} as TOptions<T>) {
  ...
  const {
    formData = {},
    maxSize = site.imageSize * 1024 * 1024,
    accept = ['*'],
    fileType = 'image',
    success,
    error: onError,
  ...
  const handleFileChoose = ({ tempFilePath, size }: { tempFilePath: string, size: number }) => {
    ...
    // const fileExtension = tempFilePath?.tempFiles?.name?.split('.').pop()?.toLowerCase()
    ...
    uploadFile({
      ...
      onSuccess: (res) => {
        ...
        data.value = res.data.links[0]
        // console.log('上传成功', res)
        success?.(res.data.links[0])
      },
      onError: (err) => {
        error.value = err
        onError?.(err)
...
async function uploadFile({
  ...
  uni.uploadFile({
    ...
    success: (uploadFileRes) => {
      try {
        const ures = JSON.parse(uploadFileRes.data) as IResData<{
          links:string
        }>
        onSuccess(ures)
      }
