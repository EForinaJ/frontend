                            ...
                            <sar-card>
                                <template #title>
                                    <view class="flex items-center gap-2">
                                        ...
                                        <sar-tag plain theme="primary" size="small">{{item.product.type}}</sar-tag>
                                    </view>
                                </template>
                                <template #extra>
                                ...
                                <view @click="handleDetail(item.id)" class="flex gap-2">
                                    ...
                                    <view class="flex flex-col items-center justify-center gap-1">
                                        <text class="text-text-primary text-xs font-700">{{item.unitPrice}}{{siteStore.siteInfo.symbol}}</text>
                                        <text class="text-text-primary text-xs font-700">*{{item.quantity}}</text>
                                    </view>
                                </view>
                                <view class="mt-20rpx flex justify-end text-xs">
                                    <text class="text-text-primary">支付金额</text>
                                    <text class="text-text-primary font-700">{{item.payAmount}}{{ siteStore.siteInfo.symbol }}</text>
                                </view>
                            </sar-card>
                        </view>
...
import OrderBar from "@/components/order-bar.vue";
import { OrderStatus } from "@/enum/status";

import useBoolean from "@/hooks/boolean";
import { useSiteStore } from "@/store/site";
import {  getWindowHeight } from '@/utils/systemInfo';
import { dialog, LoadMoreStatus, toast } from "sard-uniapp";
definePage({
   needLogin:true,
   style: {
...
const handleStatus = async (e:number) => {
    list.value = []
    query.status = e
    fetchListSetTrue()
    await  getData()
