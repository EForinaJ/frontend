  ...
  () => {
    ...
    const setTokenInfo = (val: IAuthLoginRes) => {
      ...
      if (isSingleTokenRes(val)) {
        // 单token模式
        const expireTime = now + val.expire * 1000 * 60 * 60
    ...
    /**
       ...
       * @returns 登录结果
       */
    const wxPhoneLogin = async (phoneCode:string) => {
      try {
        // 获取微信小程序手机号登录的code
        const {code} = await getWxCode()

        const res = await _wxPhoneLogin({code,phoneCode})

        // 设置绑定记录
        uni.setStorageSync('has_bound_phone', true)
